...
public class PaintPanel extends JComponent implements Serializable {
    ...
    public enum tools {OVAL ,RECTANGLE, LINE, SEGMENT, SELECT, TEXT , CIRCLE}

    //0 椭圆形 1 方形 2 自由画线 3 橡皮 4 选择 5 线段 6.圆形
    public enum cursors {
        DEFAULT, CROSS, MOVE
    }
    ...
    private class MouseHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent mouseEvent) {
            switch (tool) {
                ...
                case CIRCLE :
                    saved = false;
                    currentShape = new Circle(fore, mouseEvent.getX(), mouseEvent.getY(), stroke, fill);
                    shapes.add(currentShape);
                    redoStack.clear();
                    operationStack.push(new Operation(Operation.Type.ADD, currentShape));
                    break;
                ...
                case LINE:
                    ...
                    currentShape = new DrawLineSet(fore, mouseEvent.getX(), mouseEvent.getY(), stroke);
                    shapes.add(currentShape);
                    redoStack.clear();
                    operationStack.push(new Operation(Operation.Type.ADD, currentShape));
                    break;
    ...
    private class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent mouseEvent) {
            saved = false;
            if ((tool == tools.OVAL || tool == tools.CIRCLE ||tool == tools.RECTANGLE || tool == tools.SEGMENT) && currentShape != null) {
                ...
                x1 = x2;
                y1 = y2;
                DrawLineSet temp = (DrawLineSet) currentShape;
